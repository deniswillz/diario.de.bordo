...
export const Layout: React.FC<LayoutProps> = ({ children, currentSection, onSectionChange, user, isGuest, onLogout, data }) => {
  ...
  const navItems = [
    ...
    { id: 'comentarios', label: 'Comentários', icon: 'M8 10h.01M12 10h.01M16 10h.01M9 16H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-5l-5 5v-5z' },
  ];
  ...
  return (
    <div className="flex h-screen overflow-hidden flex-col">
      {/* HEADER AGROSYSTEM (FIEL À IMAGEM) */}
      <header className="h-14 bg-[#005c3e] flex items-center justify-between px-6 shrink-0 z-50 text-white shadow-lg">
        ...
        {/* Lado Direito: Ações e Data */}
        <div className="flex items-center gap-3">
          {/* Sino Notificação */}
          <div className="relative group">
             <button className="p-2 hover:bg-white/10 rounded-full transition-colors relative">
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9" /></svg>
                <span className="absolute top-1 right-1 w-4 h-4 bg-red-600 text-[10px] font-black flex items-center justify-center rounded-full border border-[#005c3e]">3</span>
             </button>
          </div>

          {/* Usuário e Avatar */}
          <div className="flex items-center gap-2.5 px-3 py-1.5 hover:bg-white/5 rounded-lg transition-colors cursor-pointer">
            <span className="text-[11px] font-bold hidden sm:block">Admin Agro</span>
          ...
          {/* Logout */}
          <button onClick={onLogout} className="p-2 hover:bg-white/10 rounded-lg transition-colors border border-white/10">
